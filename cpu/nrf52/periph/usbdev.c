...
#define NRFUSB_EPDATASTATUS_OUT     (USBD_EPDATASTATUS_EPOUT1_Msk |\
...
static const usbdev_driver_t driver = {
    .init = _init,
    .new_ep = _new_ep,
    .get = _get,
    .set = _set,
...
static void nrf_usbdev_setup(nrfusb_t *usbdev)
{
    usbdev->usbdev.driver = &driver;
    usbdev->device = NRF_USBD;
}

void usbdev_init_lowlevel(void)
{
    nrf_usbdev_setup(&_usbdev);
}

usbdev_t *usbdev_get_ctx(unsigned num)
{
    assert(num == 0);
    return &_usbdev.usbdev;
}

void _init(usbdev_t *dev)
{
    ...
    usbdev->device->INTEN = USBD_INTEN_USBRESET_Msk | USBD_INTEN_EPDATA_Msk | USBD_INTEN_USBEVENT_Msk;
...
bool _ep_irq_is_enabled(usbdev_ep_t *ep)
{
    nrfusb_t *usbdev = (nrfusb_t*)ep->dev;
    if (ep->dir == USB_EP_DIR_IN) {
        return usbdev->device->INTENCLR & (USBD_INTENCLR_ENDEPIN0_Msk << ep->num);
    }
    else {
        return usbdev->device->INTENCLR & (USBD_INTENCLR_ENDEPOUT0_Msk << ep->num);
    }
}
...
static void _esr(usbdev_t *dev)
{
    ...
    if (usbdev->device->EVENTS_USBEVENT) {
        printf("usbevent: %lx\n", usbdev->device->EVENTCAUSE);
        if (usbdev->device->EVENTCAUSE & USBD_EVENTCAUSE_SUSPEND_Msk) {
            usbdev->device->EVENTCAUSE = USBD_EVENTCAUSE_SUSPEND_Msk;
            usbdev->usbdev.cb(&usbdev->usbdev, USBDEV_EVENT_SUSPEND);
            puts("SUSPEND");
        }
        else if (usbdev->device->EVENTCAUSE & USBD_EVENTCAUSE_RESUME_Msk) {
            usbdev->device->EVENTCAUSE = USBD_EVENTCAUSE_RESUME_Msk;
            usbdev->usbdev.cb(&usbdev->usbdev, USBDEV_EVENT_RESUME);
            puts("RESUME");
        }
        else {
            usbdev->device->EVENTCAUSE = usbdev->device->EVENTCAUSE;
...
void isr_usbd(void)
{
    ...
    if (usbdev->device->EVENTS_USBRESET &&
        ...
        if (usbdev->device->EVENTS_EP0SETUP && (usbdev->device->INTEN & USBD_INTEN_EP0SETUP_Msk)) {
            usbdev->usbdev.epcb(_get_ep_out(usbdev, 0), USBDEV_EVENT_ESR);
            _ep_disable_irq(_get_ep_out(usbdev, 0));
        }
        if (usbdev->device->EVENTS_EP0DATADONE && (usbdev->device->INTEN & USBD_INTEN_EP0DATADONE_Msk)) {
            usbdev->usbdev.epcb(_get_ep_in(usbdev, 0), USBDEV_EVENT_ESR);
