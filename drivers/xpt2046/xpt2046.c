...
int xpt2046_get_xyz(const xpt2046_t *dev, xpt2046_xyz_t *measure)
{
    ...
    DEBUG("[xpt2046]: recv: %x, %x, %x, %x, %x, %x, %x, %x\n", recv[1], recv[2], recv[3], recv[4], recv[5], recv[6], recv[7], recv[8]);
    measure->y = ((recv[5] << 5) | (recv[6] >> 3));
    measure->x = ((recv[7] << 5) | (recv[8] >> 3));
    uint16_t z1 = ((recv[1] << 5) | (recv[2] >> 3 ));
    uint16_t z2 = ((recv[3] << 5) | (recv[4] >> 3 ));
    measure->z = (((float)z2/(float)z1) - 1.0f) * (float)(measure->x);
    return XPT2046_OK;
}
...
{
    memcpy(&dev->params, params, sizeof(xpt2046_params_t));
