 ...
 *
 * @file
 * @brief       libsodium NaCl crypto library tests
 *
...
static void test_libsodium_01(void)
{
    int res;
    ...
    TEST_ASSERT_EQUAL_INT(crypto_box(c, m, MLEN, n, bob_pk, alice_sk), 0);
...
static void test_libsodium_02(void)
{
    int res;
...
static void test_libsodium_03(void)
{
    int res;
...
Test *tests_libsodium_all(void)
{
    EMB_UNIT_TESTFIXTURES(fixtures) {
        new_TestFixture(test_libsodium_01),
        new_TestFixture(test_libsodium_02),
        new_TestFixture(test_libsodium_03)
    };

    EMB_UNIT_TESTCALLER(libsodium_tests, setUp, tearDown, fixtures);
    return (Test*)&libsodium_tests;
}

void tests_libsodium(void)
{
    if (sodium_init())
    {
        printf("Init failure in libsodium\n");
    }
    TESTS_RUN(tests_libsodium_all());
}
